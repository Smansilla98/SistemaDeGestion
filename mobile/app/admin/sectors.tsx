...
export default function AdminSectorsScreen() {
  ...
  const create = async () => {
    ...
    setBusy(true);
    try {
      await api.createCatalogSector({ name: name.trim() });
      setName('');
      await load();
    } catch (e) {
  ...
  return (
    <View style={styles.root}>
      <PageHeader title="Sectores" subtitle="Zonas de mesas" icon="map" />
      <View style={{ padding: space.md }}>
        <PrimaryButton title="Volver" variant="ghost" onPress={() => router.back()} />
        {canWrite ? (
          <>
            <SectionLabel>Nuevo sector</SectionLabel>
            <Field label="Nombre" value={name} onChangeText={setName} placeholder="Ej. Terraza" />
