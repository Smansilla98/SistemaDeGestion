...
@section('title', $type === 'INSUMO' ? 'Nuevo Insumo' : 'Nuevo Producto')

@section('content')
<div class="row mb-4">
    <div class="col-12">
        <a href="{{ route('products.index', ['type' => $type]) }}" class="btn btn-secondary mb-2">
            <i class="bi bi-arrow-left"></i> Volver
        </a>
        <h1 class="text-white mb-2" style="font-weight: 700; font-size: 2.5rem;">
            <i class="bi bi-plus-circle"></i> 
            {{ $type === 'INSUMO' ? 'Nuevo Insumo' : 'Nuevo Producto' }}
        </h1>
    </div>
</div>
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('products.store') }}" method="POST" id="productForm">
                    ...
                    @include('products._pricing-fields')
                    @else
                    @include('products._sale-price-field')
                    @endcan
                    @else
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('unit') is-invalid @enderror" id="unit" name="unit" required>
                                ...
                                <option value="litro" {{ old('unit') == 'litro' ? 'selected' : '' }}>Litro (L)</option>
                                <option value="metro" {{ old('unit') == 'metro' ? 'selected' : '' }}>Metro (m)</option>
                                <option value="rollo" {{ old('unit') == 'rollo' ? 'selected' : '' }}>Rollo</option>
                                <option value="otro" {{ old('unit') == 'otro' ? 'selected' : '' }}>Otro</option>
                            </select>
                            @error('unit')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6 mb-3" id="unit_other_group" style="display: none;">
                            <label for="unit_other" class="form-label">Especificar unidad</label>
                            <input type="text" class="form-control" id="unit_other" name="unit_other" value="{{ old('unit_other') }}" placeholder="Ej: bolsa, botella, etc.">
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" step="0.01" class="form-control @error('unit_cost') is-invalid @enderror" id="unit_cost" name="unit_cost" value="{{ old('unit_cost') }}" required min="0">
                        </div>
                        @error('unit_cost')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3 form-check">
                        <input type="checkbox" class="form-check-input" id="has_stock" name="has_stock" {{ old('has_stock') ? 'checked' : '' }}>
                        <label class="form-check-label" for="has_stock">
                            Maneja stock
                        </label>
                    </div>

                    <div class="mb-3" id="stock_minimum_group" style="display: none;">
                        <label for="stock_minimum" class="form-label">Stock Mínimo</label>
                        <input type="number" class="form-control @error('stock_minimum') is-invalid @enderror" id="stock_minimum" name="stock_minimum" value="{{ old('stock_minimum', 0) }}" min="0">
                        @error('stock_minimum')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                        <a href="{{ route('products.index', ['type' => $type]) }}" class="btn btn-secondary">Cancelar</a>
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-check-circle"></i> Crear {{ $type === 'INSUMO' ? 'Insumo' : 'Producto' }}
                        </button>
                    </div>
                </form>
            </div>
